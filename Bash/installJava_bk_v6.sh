...
echo "Install directory is present..no need to create" 
read -p "Press enter to continue"   # new lines
else
mkdir -i $USER_INSTALL_DIR
echo "New Directory Created called :" $USER_INSTALL_DIR
read -p "Press enter to continue" # new lines
fi
...
post_install(){
echo "My bin directory is :" $USER_INSTALL_DIR
echo "My Home directory is :" $HOME_DIR
#echo "export PATH=$USER_INSTALL_DIR/bin:$PATH"
echo "export PATH=$USER_INSTALL_DIR/bin:$PATH" >> $HOME_DIR/.bashrc  && source ~/.bashrc
java -version
if [ $? -eq 0 ]; then
...
do
  ...
  case $JAVA_VERSION in
    ...
    11) JAVA_DL=$(echo "https://public.dhe.ibm.com/ibmdl/export/pub/systems/cloud/runtimes/java/11.0.12.0/linux/x86_64/ibm-semeru-certified-jdk_x64_linux_11.0.12.0.bin")
        ...
        USER_INSTALL_DIR=/opt/ibm/ibm-semeru-certified-11-jdk
        #pre_req; install; post_install
        $1
        break
        ;;
